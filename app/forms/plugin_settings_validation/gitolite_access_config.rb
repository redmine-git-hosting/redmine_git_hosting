module PluginSettingsValidation
  module GitoliteAccessConfig
    ...
    included do
      ...
      validates :ssh_server_domain,   presence: true, format: { with: PluginSettingsForm::DOMAIN_REGEX }
      validates :http_server_domain,  presence: true, format: { with: PluginSettingsForm::DOMAIN_REGEX }
      validates :https_server_domain, format: { with: PluginSettingsForm::DOMAIN_REGEX }, allow_blank: true

      validates :show_repositories_url,      presence: true, inclusion: { in: PluginSettingsForm::BOOLEAN_FIELDS }
      validates :gitolite_daemon_by_default, presence: true, inclusion: { in: PluginSettingsForm::BOOLEAN_FIELDS }
