module RedmineGitolite
  ...
  class Recycle
    ...
    def self.delete_expired_files
      return unless GitHosting.file_exists?(RedmineGitolite::Config.gitolite_recycle_bin_dir)

      result = %x[#{GitHosting.shell_cmd_runner} find '#{RedmineGitolite::Config.gitolite_recycle_bin_dir}' -type d -regex '.*\.git' -cmin +#{RedmineGitolite::Config.gitolite_recycle_bin_expiration_time} -prune -print].chomp.split("\n")
      if result.length > 0
        ...
        %x[#{GitHosting.shell_cmd_runner} rmdir #{RedmineGitolite::Config.gitolite_recycle_bin_dir} 2>/dev/null]
      end
    end


    def self.move_repository_to_recycle (repository_data)
      ...
      new_path = File.join(RedmineGitolite::Config.gitolite_recycle_bin_dir,"#{Time.now.to_i.to_s}#{TRASH_DIR_SEP}#{name_to_recycle_name(repo_name)}.git")

      logger.info "Moving '#{repo_name}' to Recycle Bin..."
      logger.debug "'#{repo_path}' => '#{new_path}'"

      begin
        GitHosting.shell %[#{GitHosting.shell_cmd_runner} mkdir -p '#{RedmineGitolite::Config.gitolite_recycle_bin_dir}']
        GitHosting.shell %[#{GitHosting.shell_cmd_runner} chmod 770 '#{RedmineGitolite::Config.gitolite_recycle_bin_dir}']
      ...
      logger.info "Done !"
      logger.info "Will remain for at least #{RedmineGitolite::Config.gitolite_recycle_bin_expiration_time/60.0} hours"

      # If any empty directories left behind, try to delete them.  Ignore failure.
      old_prefix = File.dirname(repo_name) # Top-level old directory without trailing '/'
      if old_prefix
        repo_subpath = File.join(RedmineGitolite::Config.gitolite_global_storage_dir, old_prefix)
        redmine_storage = File.join(RedmineGitolite::Config.gitolite_global_storage_dir, RedmineGitolite::Config.gitolite_redmine_storage_dir)
    ...
    def self.recover_repository_if_present repo_name
      ...
      myregex = File.join(RedmineGitolite::Config.gitolite_recycle_bin_dir, "[0-9]+#{TRASH_DIR_SEP}#{name_to_recycle_name(repo_name)}.git")
      files = %x[#{GitHosting.shell_cmd_runner} find '#{RedmineGitolite::Config.gitolite_recycle_bin_dir}' -type d -regex '#{myregex}' -prune].chomp.split("\n").sort {|x,y| y <=> x }
      if files.length > 0
        # Found something!
        ...
        begin
          ...
          if prefix
            repo_prefix = File.join(RedmineGitolite::Config.gitolite_global_storage_dir, prefix)
