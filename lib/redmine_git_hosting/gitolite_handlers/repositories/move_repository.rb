      ...
      class MoveRepository < Base
        ...
        def move_physical_repo(old_path, new_path, new_parent_path)
          ...
          if new_path_exists && old_path_exists
            move_physical_repo_case_1 old_path, new_path

          ## CASE 2
          elsif !new_path_exists && old_path_exists
            move_physical_repo_case_2 old_path, new_path, new_parent_path

          ## CASE 3
          elsif !new_path_exists && !old_path_exists
            logger.error "#{context} : both old repository '#{old_path}' and new repository '#{new_path}' does not exist, " \
                         'cannot move it, exit but let Gitolite create the new repo !'
            true
        ...
        def move_physical_repo_case_1(old_path, new_path)
          if empty_repository? new_path
            ...
            logger.warn "#{context} : target repository '#{new_path}' exists and is not empty, " \
                        'considered as already moved, try to remove the old_path if empty'
