  ...
  module GitoliteWrapper
    module GitoliteProjectsHelper
      ...
      def move_physical_repo(old_path, new_path, new_parent_path)
        ...
        new_path_exists = RedmineGitHosting::GitoliteWrapper.sudo_dir_exists?(new_path)
        old_path_exists = RedmineGitHosting::GitoliteWrapper.sudo_dir_exists?(old_path)

        ## CASE 1
        if new_path_exists && old_path_exists

          if RedmineGitHosting::GitoliteWrapper.sudo_repository_empty?(new_path)
            logger.warn("#{action} : target repository '#{new_path}' already exists and is empty, remove it ...")
            begin
              RedmineGitHosting::GitoliteWrapper.sudo_rmdir(new_path, true)
            ...
            if RedmineGitHosting::GitoliteWrapper.sudo_repository_empty?(old_path)
              begin
                RedmineGitHosting::GitoliteWrapper.sudo_rmdir(old_path, true)
                return true
              rescue RedmineGitHosting::Error::GitoliteCommandException => e
        ...
        ## CASE 2
          ...
          if !RedmineGitHosting::GitoliteWrapper.sudo_dir_exists?(new_parent_path)
            begin
