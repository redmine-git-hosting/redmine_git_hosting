...
describe GitolitePublicKeysHelper do
  ...
  before(:all) do
    ...
    @gitolite_public_key = FactoryGirl.create(:gitolite_public_key, :user_id => @user.id, :key_type => 1, :title => 'foo1', :key => TEST_KEY)
  end
  ...
  describe ".keylabel" do
    ...
    context "when current user is not the key owner" do
      ...
      it { expect(helper.keylabel(@gitolite_public_key)).to eq 'user2@foo1' }
    end
  end
